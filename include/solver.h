...
void solve_block_pointing_groups_row(Grid *g, int block_y, int block_x);                // FOR A ROW: Takes a grid and the coordinates of a block, attempts to find pointing pairs or pointing triplets and then removes possibilities from corresponding cells outside of the block. For example, if we look in a block and find that only two of the unsolved squares have a possibility of being 2, if those cells are on the same row (or column), we know that there will not be a 2 on that row (or column) outside of this block.
void solve_block_pointing_groups_column(Grid *g, int block_y, int block_x);             // Same as above but for columns instead of rows
int solve_block_sum_possibilities_num(const Grid g, int block_y, int block_x, int num); // Takes grid, the coordinates of a block, and a number, returns how many cells have a possibility of being the given number
bool solve_cell_in_block(int cell_y, int cell_x, int block_y, int block_x);             // Returns true if the coordinates of the given cell are inside of the coordinates of the given block, and false if not
